...
trait NodeScala {
  ...
  private def respond(exchange: Exchange, token: CancellationToken, response: Response): Unit = {
    response takeWhile {_ => !token.isCancelled} foreach { exchange.write }
...
//    }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val sub = Future.run() { token =>
      async {
        while (!token.isCancelled) {
          await(
            listener.nextRequest() map { case (request, exchange) =>
    ...
    Subscription(listener.start(), sub)
  }
